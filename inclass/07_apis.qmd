...
# Sys.setenv(CENSUS_API_KEY = "ed3f53989d626c1da146aeab981a6551c22a9d87")
...
```{r}
# loop to grab multiple years
out <- vector("list", 5)
for(i in 1:5) {
  Sys.sleep(0.5)
  out[[i]] <- tidycensus::get_acs(
    year = i + 2016,
    state = "MN",
    geography = "tract",
    variables = c("B01003_001", "B19013_001"),
...
hennepin_over_time <- purrr::list_rbind(out) |>
  mutate(population = B01003_001E,
         median_income = B19013_001E) |>
  select(-B01003_001E, -B19013_001E, -B01003_001M, -B19013_001M)

ggplot(hennepin_over_time, aes(x = as.factor(year), y = median_income)) +
  geom_boxplot()

ggplot(hennepin_over_time, 
       aes(x = as.factor(year), y = population, group = NAME)) +
...
```{r}
choices <- function(year, county, variables) {
...
movies <- c("Coco", "Wonder+Woman", "Get+Out", 
            "The+Greatest+Showman", "Thor:+Ragnarok", "The+Princess+Diaries")
...
# Sys.setenv(NPS_KEY = "fidia4apBnRnzKMR3GaPjG54StpC396qbgyhlz89")
myapikey <- Sys.getenv("NPS_KEY")

url <- str_c("https://developer.nps.gov/api/v1/parks?parkCode=", park_code[[1]][[1]], "&api_key=", myapikey)
